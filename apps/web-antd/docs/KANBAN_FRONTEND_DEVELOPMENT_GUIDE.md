...
- `src\views\kanban\ad-cvr-optimization\index.vue`：广告低 CVR 分层优化台。运营先确认建议，再分批直接执行；关闭活动、广告组和 SP 颜色广告会暂停投放，增减预算必须输入百分比。单批最多 20 条，实时领星接口使用 5 分钟请求超时，尚未验证安全载荷的动作只允许人工处理。`execution_status=succeeded` 的行必须禁用选择，批量选择和提交参数也必须排除，失败行仍允许重试。长时执行由 `execution-task.ts` 维护跨路由状态，并通过右上角通知反馈进度，不得用模态遮罩阻塞整个系统。
- `src\api\kanban\ad-cvr-optimization.ts`：广告 CVR 总览、决策、操作上下文和领星直接写入 API。实时读写接口不得退回全局 60 秒默认超时，否则领星慢查询时可能出现后端已写入、前端却提示超时。
...
商品维度明细报表维护自己的分页、列配置、SPU、新品/老品等查询状态，但顶部时间、国家、部门、运营组、项目标签和负责人是它的全局范围，会跟随顶部筛选自动刷新。明细报表支持国家、新品/老品、运营组、SPU、负责人、展示列、固定列和分页/排序筛选；SPU 选项来自后端 `filters.spus`，请求参数为 `spus[]`。运营组使用总览接口返回的 `filters.operationGroups`。国家、新品/老品、运营组、SPU、负责人使用 Dropdown + Checkbox 的确认式筛选，不直接用普通多选 Select，避免大选项列表频繁触发表格请求。筛选浮层必须使用实体背景和足够层级，避免透明叠字。顶部 `projectTags` 必须同时透传到总览、广告监控、商品维度明细报表和内嵌新品详情表；默认剔除 `Funspread-US`，仅选择“雨靴”项目标签时允许该店铺参与计算。
...
- 按站点日期或月份查看公司经营情况。
- 展示销量完成率、销售额完成率、销售额、库存、广告花费和广告效率。
...
- 负责人列表只展示后端返回的数据，并按实际销量降序排列，销量相同按姓名稳定排序；后端过滤不在 `users` 表或状态异常的普通姓名，但可能保留按站点生成的内部部门兜底行。后端不会再因为区间目标为 0 而隐藏有实际数据或库存的成员。标题显示总展示人数和其中有销量人数。
